...
function selectPage(obj){
	...
	window.location.href="user_list4.php?page="+pagesSelect+"&action=ump&type="+UrlEncode(searchSelect) +"&keyword="+UrlEncode(txtKeyword);
}
...
function fiknode_search(){
	...
	window.location.href="user_list4.php?page=1&action=ump"+"&type="+UrlEncode(searchSelect) +"&keyword="+UrlEncode(txtKeyword);
  ...
  <tr height="500">
	  ...
	  <td valign="top">
		...
		<div id="div_search_result">	
		  <table width="800" border="0" class="dataintable">
			<tr>
				<th align="center" width="80">关联业务员</th> 
...
<?php	
	...
	if($db_link)
	{
		...
		do
		{
			...
			{
				$sql = "SELECT count(*) FROM fikcdn_client WHERE sale_group='$sale_group'"; 
			}
			else
			{
				if($sType=="username")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE username like '%$sKeyword%' AND sale_group='$sale_group';" ; 
				}
				else if($sType=="realname")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE realname like '%$sKeyword%' AND sale_group='$sale_group';"; 
				}
				else if($sType=="compname")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE company_name like '%$sKeyword%' AND sale_group='$sale_group';"; 
				}
			}
			...
			if(strlen($sKeyword)<=0)
			{
				$sql = "SELECT * FROM fikcdn_client WHERE sale_group='$sale_group' Limit $offset,$PubDefine_PageShowNum;"; 
			}
			else
			{
				if($sType=="username")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE username like '%$sKeyword%' AND sale_group='$sale_group' Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else if($sType=="realname")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE realname like '%$sKeyword%' AND sale_group='$sale_group' Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else if($sType=="compname")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE company_name like '%$sKeyword%' AND sale_group='$sale_group' Limit $offset,$PubDefine_PageShowNum;"; 
				}
			}
...
?>	 
		 ...
		 <table width="800" border="0" class="disc">
			...
			<td bgcolor="#FFFFE6"><div class="div_page_bar"> 记录总数：<?php echo $total_host;?>条&nbsp;&nbsp;&nbsp;当前第<?php echo $nPage; ?>页|共<?php echo $total_pages; ?>页&nbsp;&nbsp;&nbsp;跳转
				...
				页&nbsp;&nbsp;&nbsp;&nbsp;<a href="user_list4.php?page=1&action=first<?php echo '&type='.$sType.'&keyword='.$sKeyword; ?>">首页</a>&nbsp;&nbsp;
				<a href="user_list4.php?page=<?php echo $pageup.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pageup">上一页</a>&nbsp;&nbsp;				
				<a href="user_list4.php?page=<?php echo $pagedown.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pagedown">下一页</a>&nbsp;&nbsp;
				<a href="user_list4.php?page=<?php echo $total_pages.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=tail">尾页 </a></div></td>
			</tr>
		</table>	
	  </td> 
