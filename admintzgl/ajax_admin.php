...
if($sMod == "fiknode")
{
	if($sAction=="add")
	{
		...
		{			
			$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrConnFik,'ErrorMsg'=>'添加服务器失败，连接 15CDN 服务器错误。');
			PubFunc_EchoJsonAndExit($aryResult,$db_link);
		}
		
		if($AryResult["Return"]!="True")
		{		
			$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrReturnFalse,'ErrorMsg'=>'添加服务器失败，15CDN 管理员密码错误。');
			PubFunc_EchoJsonAndExit($aryResult,$db_link);
		}
		...
		{			
			$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrReturnFalse,'ErrorMsg'=>'添加服务器失败，获取 15CDN 授权信息错误。');
			PubFunc_EchoJsonAndExit($aryResult,$db_link);
		}
		...
		/*if(!empty($sIp)){
			$sql = "INSERT INTO fikcdn_product_node_ip(machine_room,node_ip,sale_qq,product_id)
				VALUES('$sName','$sIp','$sBackup','$sGrpid')";	
				mysql_query($sql,$db_link);
		}
		if(!empty($sUnicomIP)){
			$sql = "INSERT INTO fikcdn_product_node_ip(machine_room,node_ip,sale_qq,product_id)
				VALUES('$sName','$sUnicomIP','$sBackup','$sGrpid')";	
				mysql_query($sql,$db_link);
		}	*/
	...
	else if($sAction == "modify")
	{
		...
		$old_unicom_ip		= isset($_POST['old_unicom_ip'])?$_POST['old_unicom_ip']:'';
		$old_ip		= isset($_POST['old_ip'])?$_POST['old_ip']:'';
		$sGrpid 	= isset($_POST['grpid'])?$_POST['grpid']:'';
		$sName 		= isset($_POST['name'])?$_POST['name']:'';
		$sIp 		= isset($_POST['ip'])?$_POST['ip']:'';
		$sUnicomIP 	= isset($_POST['unicom_ip'])?$_POST['unicom_ip']:'';	
		...
		if($db_link)
		{
			...
			if(!empty($old_ip)){
				$sql="UPDATE `fikcdn_product_node_ip` SET  `machine_room`='$sName',`sale_qq`='$sBackup',`node_ip`='$sIp' WHERE `node_ip`='$old_ip'";
				 
			 mysql_query($sql,$db_link);
			}
			if(!empty($old_unicom_ip)){
				$sql="UPDATE `fikcdn_product_node_ip` SET  `machine_room`='$sName',`sale_qq`='$sBackup',`node_ip`='$sUnicomIP' WHERE `node_ip`='$old_unicom_ip'";
			 mysql_query($sql,$db_link);
			}	
			if(!mysql_query($sql,$db_link))
			{
	...
	else if($sAction == "del")
	{
		...
		{	
			...
			$sql="select ip,unicom_ip from fikcdn_node where id='$nId'";
			$result = mysql_query($sql,$db_link);
			$ip	 = mysql_result($result,0,"ip");
			$unicom_ip	 = mysql_result($result,0,"unicom_ip");
			if(!empty($ip)){
				$sql="DELETE FROM `fikcdn_product_node_ip` WHERE `node_ip`='$ip'";
			 mysql_query($sql,$db_link);
			}
			if(!empty($unicom_ip)){
				$sql="DELETE FROM `fikcdn_product_node_ip` WHERE `node_ip`='$unicom_ip'";
			 mysql_query($sql,$db_link);
			}	
			//删除自定义源站配置
...
else if($sMod == "user")
{
	...
	{		
		...
		$relation_sale= isset($_POST['relation_sale'])?$_POST['relation_sale']:'';
		...
		$sBackup 	= mysql_real_escape_string($sBackup);  	
		$relation_sale=	mysql_real_escape_string($relation_sale);
		// 是否重复添加
		...
		if(empty($relation_sale)){
			...
			$sql="SELECT belong FROM `fikcdn_admin` WHERE username='$relation_sale'";
			$result = mysql_query($sql,$db_link);
			$belong 			= mysql_result($result,0,"belong");
		}	
		
		$sql = "INSERT INTO fikcdn_client(sale_group,id,username,realname,password,relation_sale,enable_login,register_time,register_ip,addr,phone,company_name,qq,note,domain_need_verify) VALUES('$belong',NULL,'$sUsername',
				'$realname','$password','$relation_sale','$status','$add_time','$client_ip','$addr','$phone','$compname','$qq','$sBackup','$need_verify');";
	...
	else if($sAction == "modify")
	{
		...
		$compname 	= isset($_POST['compname'])?$_POST['compname']:'';
		$relation_sale 	= isset($_POST['relation_sale'])?$_POST['relation_sale']:'';
		$sale_group 	= isset($_POST['sale_group'])?$_POST['sale_group']:'';
		$phone 		= isset($_POST['phone'])?$_POST['phone']:'';
		$qq 		= isset($_POST['qq'])?$_POST['qq']:'';
		$addr 		= isset($_POST['addr'])?$_POST['addr']:'';
		$sBackup 	= isset($_POST['backup'])?$_POST['backup']:'';
		...
		{	
			...
			$compname	= mysql_real_escape_string($compname); 
			$relation_sale	= mysql_real_escape_string($relation_sale);  
			$sale_group	= mysql_real_escape_string($sale_group);
			$sBackup	= mysql_real_escape_string($sBackup);  
			$sPasswd 	= mysql_real_escape_string($sPasswd);  	
			$username 	= mysql_real_escape_string($username);  	
			$sql = "UPDATE fikcdn_client SET enable_login=$status,realname='$realname',company_name='$compname',relation_sale='$relation_sale',sale_group='$sale_group',phone='$phone',qq='$qq',note='$sBackup',addr='$addr',domain_need_verify='$need_verify' WHERE id='$uid';";
			/*var_dump($sql);
			exit();*/
			$sql1="SELECT id FROM `fikcdn_buy` WHERE username='$username'";
			 
			if(mysql_query($sql,$db_link)){
				$sql1="UPDATE `fikcdn`.`fikcdn_buy` SET `relation_sale` = '$relation_sale',sale_group='$sale_group'  WHERE `fikcdn_buy`.`username` = '$username'";
				 /*var_dump($sql1);
					exit();*/
				 $result1 = mysql_query($sql1,$db_link);
			}
			$sql2="SELECT id FROM `fikcdn_domain` WHERE username='$username'";
			if(mysql_query($sql,$db_link)){
				$sql2="UPDATE `fikcdn`.`fikcdn_domain` SET `relation_sale` = '$relation_sale',sale_group='$sale_group'  WHERE `fikcdn_domain`.`username` = '$username'";
...
else if($sMod=="admin")
{
	if($sAction=="modify")
	{
		...
		$addr 		= isset($_POST['addr'])?$_POST['addr']:'';
		$sBackup 	= isset($_POST['backup'])?$_POST['backup']:'';
		$admin_grade=isset($_POST['admin_grade'])?$_POST['admin_grade']:'';
		...
		{	
			...
			$addr	= mysql_real_escape_string($addr);  
			$sBackup	= mysql_real_escape_string($sBackup);  	
			$admin_grade=mysql_real_escape_string($admin_grade);  
			$sql = "UPDATE fikcdn_admin SET admin_grade=$admin_grade,enable=$status,nick='$realname',phone='$phone',qq='$qq',note='$sBackup',addr='$addr' WHERE id='$nid';";
			$result = mysql_query($sql,$db_link);
			if(!$result)
			{
