...
function selectPage(obj){
	...
	window.location.href="buy_list2.php?page="+pagesSelect+"&action=jump"+"&timeval="+timeSelect+"&type="+UrlEncode(searchSelect) +"&keyword="+UrlEncode(txtKeyword);
}
...
function fikcdn_search(){
	...
	window.location.href="buy_list2.php?page=1&action=jump"+"&timeval="+timeSelect+"&type="+UrlEncode(searchSelect) +"&keyword="+UrlEncode(txtKeyword);
    ...
    <td valign="top" background="../images/content-bg.gif">
	  <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				<td height="31" width="85"><a href="buy_list2.php"><span class="title_bt">未到期套餐</span></a></td>			
				<td height="31" width="85"><a href="buy_list_expire2.php"><span class="title_bt_active">已到期套餐</span></a></td>						
 				<td width="95%"></td>
			</tr>
        </table>
	   <!-- <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				<td height="31" width="85"><span class="title_bt">未到期套餐</span></td>	
				<td height="31" width="85"><span class="title_bt"><a href="buy_list2_expire.php">已到期套餐</a></span></td>
			</tr>
        </table> -->
	</td>
	...
	if($db_link)
	{
		...
		{		
			...
			if($timeval>0)
			{
				...
				$this_timeval2=time();
				/*$this_timeval2 = (-30*60*60*24);*/
				
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT count(*) FROM fikcdn_buy WHERE end_time<'$this_timeval' AND end_time>'$this_timeval2' AND end_time>unix_timestamp(now());"; 
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT count(*) FROM fikcdn_buy WHERE end_time<'$this_timeval'  AND end_time>'$this_timeval2' AND username like '%$sKeyword%' AND end_time>unix_timestamp(now());"; 
					}
				}
			}
			else
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT count(*) FROM fikcdn_buy WHERE end_time>unix_timestamp(now());"; 
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT count(*) FROM fikcdn_buy WHERE username like '%$sKeyword%' AND end_time>unix_timestamp(now());"; 
					}
				}
			}
			...
			if($timeval>0)
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_buy WHERE end_time<'$this_timeval'  AND end_time>'$this_timeval2' AND end_time>unix_timestamp(now()) ORDER BY end_time  Limit $offset,$PubDefine_PageShowNum;";
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT * FROM fikcdn_buy WHERE username like '%$sKeyword%' AND end_time<'$this_timeval'  AND end_time>'$this_timeval2' AND end_time>unix_timestamp(now()) ORDER BY end_time  Limit $offset,$PubDefine_PageShowNum;";
					}
				} 
			}
			else
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_buy WHERE end_time>unix_timestamp(now()) ORDER BY end_time  Limit $offset,$PubDefine_PageShowNum;";
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT * FROM fikcdn_buy WHERE username like '%$sKeyword%' AND end_time>unix_timestamp(now()) ORDER BY end_time  Limit $offset,$PubDefine_PageShowNum;";
					}
				}
			}
...
?>
		 ...
		 <table width="800" border="0" class="disc">
			...
			<td bgcolor="#FFFFE6"><div class="div_page_bar"> 记录总数：<?php echo $total_host;?>条&nbsp;&nbsp;&nbsp;当前第<?php echo $nPage; ?>页|共<?php echo $total_pages; ?>页&nbsp;&nbsp;&nbsp;跳转
				...
				页&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="buy_list2.php?page=1&action=first&timeval=<?php echo $timeval.'&type='.$sType.'&keyword='.$sKeyword; ?>">首页</a>&nbsp;&nbsp;
				<a href="buy_list2.php?page=<?php echo $pageup.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pageup&timeval=<?php echo $timeval; ?>">上一页</a>&nbsp;&nbsp;				
				<a href="buy_list2.php?page=<?php echo $pagedown.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pagedown&timeval=<?php echo $timeval; ?> ">下一页</a>&nbsp;&nbsp;
				<a href="buy_list2.php?page=<?php echo $total_pages.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=tail&timeval=<?php echo $timeval; ?>">尾页 </a></div></td>
			</tr>
		</table>	
	  </td> 
