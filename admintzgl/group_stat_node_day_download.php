...
function getNodeBandwidthStatData(group_id,timeval)
{
	...
	var postUrl = "request_stat_data1.php?mod=realtime&action=DayDownloadCount"+"&group_id=" + group_id +"&timeval="+timeval;
...
function OnSelectNode()
{
	...
	window.location.href="group_stat_node_day_download.php?id="+txtSelectNode+"&date2="+nDateSelect2;
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#F8F9FA">
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				...
				<td height="31" width="85"><a href="group_stat_node_day_download.php?id=<?php echo $group_id; ?>"><span class="title_bt">日流量统计</span></a></td>				
  ...
  <tr height="500">
	...
	/*	$timenow=time();
		...
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up),avg(bandwidth_down),avg(bandwidth_up) FROM realtime_list WHERE group_id=$group_id AND time>=$timeval";
		...
		*/
		...
		$sql = "SELECT max(bandwidth_down),max(bandwidth_up) FROM realtime_list WHERE group_id=$group_id AND time>=$timeval";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
...
<?php			
			...
			if($db_link){
				...
				$sql = "SELECT time,node_id,sum(user_down) user_down,sum(user_up) user_up,sum(upstream_down) upstream_down,sum(upstream_up) upstream_up FROM realtime_list_day where group_id='$group_id' AND time>='$timeval1' GROUP BY node_id order by time";
