...
function FikCdn_ModifyNode(nNodeId){
	...
	var old_unicom_ip		 =document.getElementById("old_unicom_ip").value;
	...
	var postStr="id="+txtId+"&old_ip=" + UrlEncode(old_ip) +"&old_unicom_ip=" + UrlEncode(old_unicom_ip) +"&ip=" + UrlEncode(txtIP) + "&grpid=" + grpSelect + "&unicom_ip="+UrlEncode(txtUnicomIP) + "&name=" + UrlEncode(txtNodeName) + "&allowbw=" + UrlEncode(txtAllowBW) + "&adminport=" + txtAdminPort + "&passwd=" + UrlEncode(txtPasswd) + "&backup=" + UrlEncode(txtBackup);					 				 
...
?>
				...
				<input id="old_ip" name="old_ip" type="hidden"  value="<?php echo $ip; ?>" /> 
				<input id="old_unicom_ip" name="old_unicom_ip" type="hidden"  value="<?php echo $unicom_ip; ?>" /> 
		</td>
  </tr>
  <tr>
