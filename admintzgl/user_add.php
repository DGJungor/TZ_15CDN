...
function fiknode_AddUser(){
	...
	var relation_sale =document.getElementById("relation_sale").value;
	...
	if (relation_sale.length==0 ){ 
	  	document.getElementById("tipsrelation_sale").innerHTML="请输入关联业务员";
		document.getElementById("relation_sale").focus();
	  	return false;
	}
	else
	{
		document.getElementById("tipsrelation_sale").innerHTML="";
	}
	...
	var postStr="username="+UrlEncode(txtUsername)+"&status=" + UrlEncode(statusSelect) + "&password=" + UrlEncode(hex_md5(txtPasswd)) + "&realname=" + UrlEncode(txtRealname)
					 ...
					 + "&backup=" + UrlEncode(txtBackup)+ "&relation_sale=" + UrlEncode(relation_sale);					 
	  ...
	  <td valign="top">
		  <table width="800" border="0" class="dataintable">
    		...
    		<tr bgcolor="#FFFFFF" onMouseMove="Event_trOnMouseOver(this)" onMouseOut="Event_trOnMouseOut(this)">
				...
				<td><input id="relation_sale" name="relation_sale" type="text" size="26" maxlength="32"  /> <span class="input_tips_txt" id="tipsrelation_sale" name="tipsrelation_sale" ></span>  </td>
			</tr>	
			<tr bgcolor="#FFFFFF" onMouseMove="Event_trOnMouseOver(this)" onMouseOut="Event_trOnMouseOut(this)">
