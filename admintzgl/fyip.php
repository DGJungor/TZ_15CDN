...
<head>
<title>腾正网络安全管理系统</title>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
<meta name="description" content="腾正网络安全管理系统" />
<meta name="keywords" content="腾正网络安全管理系统" />
<?php
...
function getNodeBandwidthStatData(node_id,timeval)
{
	...
	var postUrl = "fy_data.php?mod=realtime&action=bandwidth"+"&node_id=" + node_id +"&timeval="+timeval;
...
function OnSelectNode()
{
	...
	window.location.href="fyip.php?id="+txtSelectNode+"&date2="+nDateSelect2;
}
...
<?php  
	...
	if($db_link)
	{
		...
		$sql = "SELECT * FROM fw_vip where id='$node_id';"; 
		...
		if($result)
		{
			...
			for($i=0;$i<$row_count;$i++)
			{
				...
				$node_ip 		= mysql_result($result,$i,"vip_ip");
				$node_unicom_ip = mysql_result($result,$i,"unicom_ip");
				$node_name  	= mysql_result($result,$i,"bz");
				$group_id 		= mysql_result($result,$i,"vip_in_m");
				
				if(strlen($node_ip)<=0) $node_ip = $this_node_id;
				...
				if($node_ip==$node_ip)
				{
					echo '<option value="'.$node_ip.'" selected="selected" >'.$show_name.'</option>';
					
					$show_this_name = $sFikIP;	
				}
				else
				{
					echo '<option value="'.$node_ip.'" >'.$show_name.'</option>';
				}
			}
		}
		...
		$sql = "SELECT max(bandwidth_down),avg(bandwidth_down),max(upstream_bandwidth_up),avg(upstream_bandwidth_up) FROM fyip WHERE node_id='$node_id' AND time>=$timeval";
		...
		if($result2 && mysql_num_rows($result2)>0)
		{
			...
			$max1_upstream_bandwidth_up = mysql_result($result2,0,"max(upstream_bandwidth_up)");
			$avg1_upstream_bandwidth_up = mysql_result($result2,0,"avg(upstream_bandwidth_up)");		
			
			$max1_bandwidth_down = round($max1_bandwidth_down,2);
			$avg1_bandwidth_down = round($avg1_bandwidth_down,2);
			$max1_upstream_bandwidth_up = round($max1_upstream_bandwidth_up,2);
			$avg1_upstream_bandwidth_up = round($avg1_upstream_bandwidth_up,2);									
		}
		
		if(strlen($max1_bandwidth_down)<=0) $max1_bandwidth_down=0;
		if(strlen($max1_upstream_bandwidth_up)<=0) $max1_upstream_bandwidth_up=0;
		if(strlen($avg1_bandwidth_down)<=0) $avg1_bandwidth_down=0;
		if(strlen($avg1_upstream_bandwidth_up)<=0) $avg1_upstream_bandwidth_up=0;			
		
		// 计算最大带宽
		$timeval = $timenow-24*60*60*3;
		$sql = "SELECT max(bandwidth_down),max(upstream_bandwidth_up) FROM fyip WHERE node_id='$node_id' AND time>=$timeval";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
			$max7_bandwidth_down = mysql_result($result2,0,"max(bandwidth_down)");
			$max7_upstream_bandwidth_up = mysql_result($result2,0,"max(upstream_bandwidth_up)");		
	
			$max7_bandwidth_down = round($max7_bandwidth_down,2);
			$max7_upstream_bandwidth_up = round($max7_upstream_bandwidth_up,2);								
		}	
...
?>
		...
		<input id="txtNodeID" type="hidden" size="20" maxlength="256" value="<?php echo $node_ip; ?>" />
		...
		<table width="940" border="0" cellspacing="0" cellpadding="0" style="padding-left:45px">
			...
			<tr>
				<td class="title_line" width="20%" height="25" align="right" style="background-color: whitesmoke"><strong>24小时最大被攻击流量：</strong></td>
				...
				<td class="title_line" width="20%" height="25" align="right" style="background-color: whitesmoke"><strong>24小时被牵引次数：</strong></td>
				<td class="title_line" width="10%" height="25" align="right" style="padding-right: 10px;"><span id="tb_max_mirror_flow_of_1day">跨库查询</span></td>				
				<td class="title_line" width="18%" height="25" align="right" style="background-color: whitesmoke"><strong>3天内最大被攻击流量：</strong></td>
			...
			<tr>
				<td class="title_line" width="20%" height="25" align="right" style="background-color: whitesmoke"><strong>24小时最大业务带宽：</strong></td>
				<td class="title_line" width="10%" height="25" align="right" style="padding-right: 10px;"><span id="tb_avg_mirror_flow_of_1day"><?php echo $max1_upstream_bandwidth_up; ?> Mbps</span></td>
				<td class="title_line" width="20%" height="25" align="right" style="background-color: whitesmoke"><strong>24小时平均业务带宽：</strong></td>
				<td class="title_line" width="10%" height="25" align="right" style="padding-right: 10px;"><span id="tb_avg_mirror_flow_of_1day"><?php echo $avg1_upstream_bandwidth_up; ?> Mbps</span></td>				
				<td class="title_line" width="18%" height="25" align="right" style="background-color: whitesmoke"><strong>3天内最大业务带宽：</strong></td>
				<td class="title_line" width="10%" height="25" align="right" style="padding-right: 10px;"><span id="tb_avg_mirror_flow_of_7day"><?php echo $max7_upstream_bandwidth_up; ?> Mbps</span></td>
			</tr>
		</table>
		...
		<div id="div_search_result2">
			<table width="800" border="0" class="dataintable" id="domain_table">
				<tr id="tr_domain_title">
					...
					<th align="right" width="100">流攻击流量带宽</th>			
					<th align="right" width="100" align="center">正常业务带宽</th>		
				</tr>	
<?php				
			...
			if($db_link){
				...
				$sql = "SELECT * FROM fyip where node_id='$node_id' AND time>='$timeval1' AND time<'$timeval2' ORDER BY time DESC"; 
				$result = mysql_query($sql,$db_link);
				if($result){
					$row_count=mysql_num_rows($result);
					for($i=0;$i<$row_count;$i++){
