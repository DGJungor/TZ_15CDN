...
function getNodeBandwidthStatData(group_id,timeval)
{
	var xmlhttp;
	...
	var postUrl = "request_stat_data1.php?mod=realtime&action=bandwidth_max"+"&group_id=" + group_id +"&timeval="+timeval;
...
function OnSelectNode()
{
	...
	window.location.href="group_stat_node_bandwidth_max.php?id="+txtSelectNode+"&date2="+nDateSelect2;
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#F8F9FA">
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				...
				<td height="31" width="85"><a href="group_stat_node_bandwidth_max.php?id=<?php echo $group_id; ?>"><span class="title_bt">峰值带宽</span></a></td>
				...
				<td height="31" width="85"><a href="group_stat_node_hostorder.php?id=<?php echo $group_id; ?>"><span class="title_bt_active">最新排名</span></a></td>
				<td width="95%"></td>
			</tr>
        </table>
	</td>
  ...
  <tr height="500">
	...
	if($db_link)
	{
		...
		$sql = "SELECT max(a.bandwidth_down) max_bandwidth_down ,max(a.bandwidth_up) max_bandwidth_up,avg(a.bandwidth_down) avg_bandwidth_down,avg(a.bandwidth_up) avg_bandwidth_up FROM (SELECT sum(bandwidth_down) bandwidth_down,sum(bandwidth_up) bandwidth_up FROM realtime_list WHERE group_id=$group_id  AND time>=$timeval group by node_id ) a";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
			$max1_bandwidth_down = mysql_result($result2,0,"max_bandwidth_down");
			$max1_bandwidth_up = mysql_result($result2,0,"max_bandwidth_up");		
			$avg1_bandwidth_down = mysql_result($result2,0,"avg_bandwidth_down");
			$avg1_bandwidth_up = mysql_result($result2,0,"avg_bandwidth_up");		
		...
		$sql = "SELECT max(a.bandwidth_down) max_bandwidth_down ,max(a.bandwidth_up) max_bandwidth_up  FROM (SELECT sum(bandwidth_down) bandwidth_down,sum(bandwidth_up) bandwidth_up FROM realtime_list WHERE group_id=$group_id  AND time>=$timeval group by node_id ) a";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
			$max7_bandwidth_down = mysql_result($result2,0,"max_bandwidth_down");
			$max7_bandwidth_up = mysql_result($result2,0,"max_bandwidth_up");		
...
?>
		...
		<input id="txtNodeID" type="hidden" size="20" maxlength="256" value="<?php echo $group_id; ?>" />
...
<?php				
			if($db_link){
				...
				$sql = "SELECT time,node_id,sum(down_increase) down_increase,sum(up_increase) up_increase,sum(upstream_down_increase) upstream_down_increase,sum(upstream_up_increase) upstream_up_increase,sum(bandwidth_down) bandwidth_down,sum(bandwidth_up) bandwidth_up,sum(upstream_bandwidth_down) upstream_bandwidth_down,sum(upstream_bandwidth_up) upstream_bandwidth_up FROM realtime_list_max where group_id='$group_id' AND time>='$timeval1' AND time<'$timeval2' GROUP BY node_id ORDER BY time"; 
				$result = mysql_query($sql,$db_link);
				if($result){
					$row_count=mysql_num_rows($result);
					for($i=0;$i<$row_count;$i++){
...
getNodeBandwidthStatData(<?php echo $group_id; ?>,1);
</script>
<?php
