...
function selectPage(obj){
	...
	window.location.href="domain_list4.php?page="+pagesSelect+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect+"&selectOrder="+selectOrder;
}
...
function FikHost_Search(){
	...
	window.location.href="domain_list4.php?page=1"+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect;
...
function selectGroup(){
	...
	var txtBuyId		 =document.getElementById("grpSelect").value;
	var selectOrder =document.getElementById("selectOrder").value;
	window.location.href="domain_list4.php?page=1"+"&action=jump"+"&buy_id="+txtBuyId+"&keyword="+UrlEncode(txtKeyword)+"&type="+searchSelect+"&selectOrder="+selectOrder;
}
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				<td height="31" width="85"><a href="domain_list4.php?buy_id=<?php echo $buy_id.'&page='.$nPage.'&action='.$action.'&type='.$sType.'&keyword='.$sKeyword.'&selectOrder='.$selectoder; ?>"><span class="title_bt">域名列表</span></a></td>
				<td height="31" width="85"><a href="domain_list_info4.php?buy_id=<?php echo $buy_id.'&page='.$nPage.'&action='.$action.'&type='.$sType.'&keyword='.$sKeyword.'&selectOrder='.$selectoder; ?>"><span class="title_bt_active">更多信息</span></a></td>
	...
	{	
		...
		if($result)
		{
			...
			for($i=0;$i<$row_count;$i++)
			{
				...
				$sale_group=mysql_result($result,$i,"sale_group");
				...
				{						
					...
					if($_SESSION['sale_group']==$sale_group){
...
<?php
		$sale_group=$_SESSION['sale_group'];
	if(empty($sale_group)){
		exit();
	}
	if(!is_numeric($nPage))
	...
	if($db_link)
	{
		do
		{
			...
			if($buy_id=="all" || strlen($buy_id)<=0)
			{
				...
				{
					$sql = "SELECT count(*) FROM fikcdn_domain WHERE sale_group='$sale_group';"; 
				}
				else
				{
					if($sType=="domain")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE hostname like '%$sKeyword%' AND sale_group='$sale_group'"; 
					}
					else if($sType=="srcip")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE upstream like '%$sKeyword%' AND sale_group='$sale_group'";
					}						
					else if($sType=="srcip2")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE unicom_ip like '%$sKeyword%' AND sale_group='$sale_group'";
					}		
					else if($sType=="owner")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE username like '%$sKeyword%' AND sale_group='$sale_group'"; 
					}
				}
			}
			else
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT count(*) FROM fikcdn_domain WHERE buy_id='$buy_id' AND sale_group='$sale_group';"; 
				}
				else
				{
					if($sType=="domain")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE buy_id='$buy_id' AND hostname like '%$sKeyword%' AND sale_group='$sale_group'"; 
					}
					else if($sType=="srcip")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE buy_id='$buy_id' AND upstream like '%$sKeyword%' AND sale_group='$sale_group'";
					}	
					else if($sType=="srcip2")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE buy_id='$buy_id' AND unicom_ip like '%$sKeyword%' AND sale_group='$sale_group'";
					}				
					else if($sType=="owner")
					{
						$sql = "SELECT count(*) FROM fikcdn_domain WHERE buy_id='$buy_id' AND username like '%$sKeyword%' AND sale_group='$sale_group'"; 
					}
				}
			}
			...
			if($buy_id=="all" || strlen($buy_id)<=0)
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_domain WHERE sale_group='$sale_group'order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else
				{
					if($sType=="domain")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE hostname like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}
					else if($sType=="srcip")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE upstream like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}	
					else if($sType=="srcip2")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE unicom_ip like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}					
					else if($sType=="owner")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE username like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}
				}
			}
			else
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_domain WHERE buy_id='$buy_id' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else
				{
					if($sType=="domain")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE buy_id='$buy_id' AND hostname like '%$sKeyword%'  AND sale_group='$sale_group'order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}
					else if($sType=="srcip")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE buy_id='$buy_id' AND upstream like '%$sKeyword%'  AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}	
					else if($sType=="srcip2")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE buy_id='$buy_id' AND unicom_ip like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}				
					else if($sType=="owner")
					{
						$sql = "SELECT * FROM fikcdn_domain WHERE buy_id='$buy_id' AND username like '%$sKeyword%' AND sale_group='$sale_group' order by $selectoder Limit $offset,$PubDefine_PageShowNum;"; 
					}
				}
			}
			...
			for($i=0;$i<$row_count;$i++)
			{
				...
				else if(strlen($upstream)<=0)
				{
					...
					echo '<td>  <a href="stat_domain_bandwidth4.php?domain_id='.$id.'" onclick="javescript:FikCdn_SelectDomainStat();" title="查看此域名流量统计信息">流量统计</a>&nbsp;</td>';
...
?>
		 ...
		 <table width="800" border="0" class="disc">
			...
			<td bgcolor="#FFFFE6"><div class="div_page_bar"> 记录总数：<?php echo $total_host;?>条&nbsp;&nbsp;&nbsp;当前第<?php echo $nPage; ?>页|共<?php echo $total_pages; ?>页&nbsp;&nbsp;&nbsp;跳转
				...
				页&nbsp;&nbsp;&nbsp;&nbsp;
				<a href="domain_list4.php?page=1&action=first&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">首页</a>&nbsp;&nbsp;
				<a href="domain_list4.php?page=<?php echo $pageup; ?>&action=pageup&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">上一页</a>&nbsp;&nbsp;				
				<a href="domain_list4.php?page=<?php echo $pagedown; ?>&action=pagedown&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">下一页</a>&nbsp;&nbsp;
				<a href="domain_list4.php?page=<?php echo $total_pages; ?>&action=tail&buy_id=<?php echo $buy_id.'&keyword='.$sKeyword.'&type='.$sType.'&selectOrder='.$selectoder; ?>">尾页 </a></div></td>
			</tr>
		</table>	
