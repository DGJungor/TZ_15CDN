...
function FikCdn_AddProductBox()
{
	...
	showMSGBOX('',520,400,BT,BL,120,boxURL,'创建套餐:');	
}
			...
			for($i=0;$i<count($arr);$i++)
			{
				...
				echo "<tr name='".$arr[$i]['p_id']."' style='display:none;' bgcolor='#FFFFFF' align='center' >";
				 ...
				 echo "<td align='left' >&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<img src='../images/menu_minus.gif' width='9' height='9' border='0' style='margin-left:0em' onclick='decrease(".$arr[$i]['p_id'].")'>
						<span><a href='product_node_ip.php?product_id=".$arr[$i]['id']."&product_name=".$arr[$i]['name']."'>".$arr[$i]['name']."</a></span></td>";}
				  else{
				...
				echo '<td><a href="javascript:void(0);" onclick="javescript:FikCdn_ModifyProductBox('.$arr[$i]['id'].');" title="修改产品套餐信息">修改</a>&nbsp;';
				echo '<a href="javascript:void(0);" onclick="javescript:FikCdn_DelProductBox('.$arr[$i]['id'].');" title="删除产品套餐信息">删除</a></td>';
				echo '</tr>';
			}
		}while(0);
