...
if($sMod=='buy')
{
	if($sAction=="del")
	{
		...
		if($db_link)
		{
			...
			//add bendy
			$sSkip 	 = isset($_POST['skip'])?$_POST['skip']:'';
			if ($sSkip <> 1){
				$sql = "SELECT * FROM fikcdn_buy WHERE id=$buy_id;";
				$result = mysql_query($sql,$db_link);
				if(!$result || mysql_num_rows($result)<=0)
				{
					$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrQuery,'ErrorMsg'=>'删除用户购买的产品套餐失败，找不到数据库记录。');
					PubFunc_EchoJsonAndExit($aryResult,$db_link);
				}
				...
				if ($dns_api_message['code'] <> 1){
					//error
					$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrQuery,'ErrorMsg'=>'无法连接DNS服务商,请稍候重试,或者联系管理员。'.$dns_api_message['message']);
					PubFunc_EchoJsonAndExit($aryResult,$db_link);
				}
				$records=$dns_api_message['data'];
				$find_host=false;
				$code=explode('.',$dns_cname)[0];
				foreach ($records as $r){
					if ($code == $r['host']) {
						//fined! remove!!
						$find_host=True;
						$dns_api_message=json_decode($dns_api->record->recordDelete($r['record_id'],$xns['domainid']),true);
						if ($dns_api_message['code'] <> 1){
							$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrNoUser,'ErrorMsg'=>'无法删除DNS记录。'.$dns_api_message['message']);
							PubFunc_EchoJsonAndExit($aryResult,$db_link);
						}
						break;
					}
				...
				if ($find_host == False){
					$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrNoUser,'ErrorMsg'=>'找不到DNS记录。'.$dns_api_message['message']);
					PubFunc_EchoJsonAndExit($aryResult,$db_link);
				}
			}
			//end bendy
