		...
		{	
			...
			echo '<table width="800" border="0" class="dataintable">
					<tr>
						...
						<th align="center" width="140" align="center">15CDN 版本</th>
			...
			if($result)
			{
				...
				if($row_count>0)
				{
					for($i=0;$i<$row_count;$i++)
					{
						...
						echo '<tr bgcolor="#FFFFFF" align="center" onMouseMove="Event_trOnMouseOver(this)" onMouseOut="Event_trOnMouseOut(this)">';
						echo '<td>'.$id.'</td>';
						echo '<td><a href="login_fikker.php?mod=logging&action=redirect&id='.$id.'" title="进入 15CDN 管理后台" target="_blank" >'.$ip.'</a></td>';
						echo '<td><a href="login_fikker.php?mod=logging&action=redirect&id='.$id.'" title="进入 15CDN 管理后台" target="_blank" >'.$unicom_ip.'</a></td>';								
						echo '<td>'.$name.'</td>';
