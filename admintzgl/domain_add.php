...
function FikCdn_AddDomain(){
	...
	var txtclient_username    	=document.getElementById("txtclient_username").value;
	...
	var postStr="buy_id="+UrlEncode(productSelect)+"&domain=" + UrlEncode(txtDomain) + "&SSLOpt=" + SSLOpt + "&SSLCrtContent=" + UrlEncode(SSLCrtContent) + "&SSLKeyContent=" + UrlEncode(SSLKeyContent) + "&SSLExtraParams=" + UrlEncode(SSLExtraParams) + 
			...
			"&icp=" + UrlEncode(txtICP) +"&dns_name=" + UrlEncode(txtDNSName) +"&backup=" + UrlEncode(txtBackup)+"&client_username=" + UrlEncode(txtclient_username);		
...
<table width="550" border="0" align="center" cellpadding="0" cellspacing="0" class="normal">
  <tr>
    <td width="140" height="40" class="objTitle" title="" ><span class="input_red_tips">*</span>域名：</td>
    <td width="320">
		<input id="txtDomain" type="text" size="36" maxlength="128" class="inputText" style="width:320px;height:25px" /> <span class="input_tips_txt" id="tipsDomain" name="tipsDomain" ></span>
	</td>
  </tr>
  <tr>
 ...
 <tr id="SSLAllParams" style="display:none">
  ...
  <tr>
    ...
    <td>
		<input id="txtSrcIP" type="text" size="36" class="inputText" maxlength="64" style="width:320px;height:25px" title="如果有多个电信源站，可用“;”间隔，如：211.155.23.66;234.34.55.123" /> <span class="input_tips_txt" id="tipsSrcIP" name="tipsSrcIP" ></span>  
	</td>
  </tr>
  <tr>
    <td height="6" colspan="2"></td>
  ...
  <tr>
    ...
    <td>
		<input id="txtUnicomIP" type="text" size="36" class="inputText" maxlength="64" style="width:320px;height:25px"  title="如果有多个联通源站，可用“;”间隔，如：211.155.23.66;234.34.55.123"  /><span class="input_tips_txt4" id="tipsUnicomIP" ></span>
	</td>
  </tr>
  <tr>
    <td height="6" colspan="2"></td>
 ...
 <tr><td height="2" colspan="2"></td></tr>
  ...
  <tr>
    ...
    <td>
		<input id="txtICP" type="text" size="36" class="inputText" maxlength="64" style="width:320px;height:25px" title="仅作为记录域名的 ICP 备案号，方便管理。" /> <span class="input_tips_txt" id="tipsICP" ></span>
	</td>
  </tr>
  <tr>
    <td height="6" colspan="2"></td>
		...
		do
		{
			...
			for($i=0;$i<$row_count;$i++)
			{
				...
				if($result2 && mysql_num_rows($result2)>0)
				{
					...
					$buy_id= mysql_result($result2,0,"id");
					 
						echo '<option value="'.$product_id.'">'.$product_name.'('.$this_username.")</option>";				
 ...
 ?>
  ...
  <tr>
    <td colspan="2">
	    <center><input id="txtclient_username"  type="hidden" value="<?php echo $this_username?>" /><input name="btnAddDomain"  type="submit" style="width:95px;height:28px" id="btnAddDomain" value="保存" style="cursor:pointer;" onclick="FikCdn_AddDomain();" /></center></td>
  </tr>
</table>
<table width="550" border="0">
	<tr>
	<td><p style="padding-left:25px;">同步说明：<br />
	1. 域名添加成功后，需要管理员审核通过才最终通过“<a href="javascript:void(0);" onclick="javescript:FikCdn_ToTaskList();" title="查看后台任务列表">后台任务</a>"方式同步到所有 15CDN 节点服务器的主机管理中；<br />
	</p></td>
	</tr>
</table>
