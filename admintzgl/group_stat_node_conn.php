...
function getConnectCountStatData(group_id,timeval)
{
	var xmlhttp;
	...
	var postUrl = "request_stat_data1.php?mod=realtime&action=connect"+"&group_id=" + group_id +"&timeval="+timeval;
...
function OnSelectNode()
{
	...
	window.location.href="group_stat_node_conn.php?id="+txtSelectNode;
}
    ...
    <td valign="top" background="../images/content-bg.gif">
	   <table width="100%" height="31" border="0" cellpadding="0" cellspacing="0" class="left_topbg" id="table2" >
			<tr height="31" class="TabToolTitle">
				...
				<td height="31" width="85"><a href="group_stat_node_conn.php?id=<?php echo $group_id; ?>"><span class="title_bt">连接数统计</span></a></td>				
...
<?php  
	...
	if($db_link)
	{
	 ...
	 $timenow=time();
		...
		$sql = "SELECT max(a.CurrentUserConnections) max_CurrentUserConnections,max(a.CurrentUpstreamConnections) max_CurrentUpstreamConnections,avg(a.CurrentUserConnections) avg_CurrentUserConnections,avg(a.CurrentUpstreamConnections) avg_CurrentUpstreamConnections FROM(SELECT sum(CurrentUserConnections) CurrentUserConnections,sum(CurrentUpstreamConnections) CurrentUpstreamConnections FROM realtime_list WHERE group_id=$group_id  AND time>=$timeval  group by node_id) a";
		$result2 = mysql_query($sql,$db_link);
		if($result2 && mysql_num_rows($result2)>0)
		{
			$max1_user_connections= mysql_result($result2,0,"max_CurrentUserConnections");
			$max1_upstream_connections = mysql_result($result2,0,"max_CurrentUpstreamConnections");	
			$avg1_user_connections= mysql_result($result2,0,"avg_CurrentUserConnections");
			$avg1_upstream_connections = mysql_result($result2,0,"avg_CurrentUpstreamConnections");
		...
		if($result2 && mysql_num_rows($result2)>0)
		{
			$max7_user_connections = mysql_result($result2,0,"max_CurrentUserConnections");
			$max7_upstream_connections = mysql_result($result2,0,"max_CurrentUpstreamConnections");								
		}	
...
getConnectCountStatData(<?php echo $group_id; ?>,1);	
