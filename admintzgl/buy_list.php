...
var __nBuyID;
var __nSKIP;
function FikCdn_DelBuyProductBox(buy_id,skip){
	__nBuyID = buy_id;
	__nSKIP  = skip;
...
function FikCdn_DelBuyProduct(){
	...
	var postStr="buy_id="+__nBuyID+"&skip="+__nSKIP;	
	...
	if($db_link)
	{
		...
		{		
			...
			if($timeval>0)
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_buy WHERE end_time<'$this_timeval'  AND end_time>'$this_timeval2' ORDER BY end_time DESC Limit $offset,$PubDefine_PageShowNum;";
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT * FROM fikcdn_buy WHERE username like '%$sKeyword%' AND end_time<'$this_timeval'  AND end_time>'$this_timeval2' ORDER BY end_time DESC Limit $offset,$PubDefine_PageShowNum;";
					}
				} 
			}
			else
			{
				if(strlen($sKeyword)<=0)
				{
					$sql = "SELECT * FROM fikcdn_buy ORDER BY end_time DESC Limit $offset,$PubDefine_PageShowNum;";
				}
				else
				{
					if($sType=="username")
					{
						$sql = "SELECT * FROM fikcdn_buy WHERE username like '%$sKeyword%' ORDER BY end_time DESC Limit $offset,$PubDefine_PageShowNum;";
					}
				}
			}
			...
			for($i=0;$i<$row_count;$i++)
			{
				...
				*/
				...
				echo '<td><a href="javascript:void(0);" onclick="javescript:FikCdn_ModifyBuyProductBox('.$id.');" title="修改已售出的套餐">修改</a>&nbsp;
					...
					<a href="javascript:void(0);" onclick="javescript:FikCdn_DelBuyProductBox('.$id.',0);" title="删除已售出的套餐">删除</a>&nbsp;
					<a href="javascript:void(0);" onclick="javescript:FikCdn_DelBuyProductBox('.$id.',1);" title="删除已售出的套餐(不更新DNS)">删除2</a>&nbsp;
