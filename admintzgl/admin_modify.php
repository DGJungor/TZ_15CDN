...
function FikCdn_ModifyAdmin(nAdminId){
	...
	var txtadmin_grade=document.getElementById("txtadmin_grade").value;
	...
	var postStr="id="+nAdminId+"&status=" + UrlEncode(statusSelect)+ "&admin_grade=" + UrlEncode(txtadmin_grade) + "&realname=" + UrlEncode(txtRealname)+ "&phone=" + UrlEncode(txtPhone) +
...
<?php
	...
	if($db_link)
	{
		do
		{
			...
			/*var_dump($id);
			exit();	*/
			$admin_grade  			= mysql_result($result,$i,"admin_grade");
...
<table width="500" border="0" align="center" cellpadding="0" cellspacing="0" class="normal">
  ...
  <tr>
    <td height="25" class="objTitle" >权限管理：</td>
    <td>
		<input id="txtadmin_grade" type="text" size="26" maxlength="32" value="<?php echo $admin_grade; ?>" /><span>1:超管;2:售后;3:售前;4:业务经理</span>
	</td>
  </tr>
  <tr>
