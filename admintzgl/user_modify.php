...
function FikCdn_ModifyUser(uid){
	...
	var txtRelation_sale  =document.getElementById("txtRelation_sale").value;
	var txtSale_group  =document.getElementById("txtSale_group").value;
	...
	var postStr="id="+txtId+"&status=" + UrlEncode(statusSelect) +"&username="+UrlEncode(txtusername)+ "&realname=" + UrlEncode(txtRealname) + "&passwd="+UrlEncode(md5Passwd) +  
			   "&compname=" + UrlEncode(txtCompName)+"&relation_sale=" + UrlEncode(txtRelation_sale)+"&sale_group=" + UrlEncode(txtSale_group) + "&phone=" + UrlEncode(txtPhone) + "&qq=" + UrlEncode(txtQQ) + "&need_verify=" + verifySelect +
...
<table width="500" border="0" align="center" cellpadding="0" cellspacing="0" class="normal">
  ...
  <input id="txtId" name="txtId" type="hidden"  value="<?php echo $uid; ?>" /> 
    <td height="25" class="objTitle" title="" >登录用户名：</td>
    <td>
		<input id="txtusername" name="txtusername" type="text" size="26" maxlength="32" value="<?php echo $username; ?>" />
	</td>
  </tr>
  ...
  <tr>
    ...
    <tr>
    <td height="25" class="objTitle" title="" >关联业务员：</td>
    <td>
		<input id="txtRelation_sale" name="txtRelation_sale" type="text" size="20" maxlength="36"  value="<?php echo $relation_sale; ?>"  />
	</td>
  </tr> 
  ...
  <tr>
    ...
    <tr>
    <td height="25" class="objTitle" title="" >业务员分组：</td>
    <td>
		<input id="txtSale_group" name="txtSale_group" type="text" size="5" maxlength="36"  value="<?php echo $sale_group; ?>"  /><span>A或者B</span>
	</td>
  </tr> 
  <tr>
    <td height="6" colspan="2"></td>
