...
if($sMod=="domain")
{
	if($sAction=="add")
	{
		...
		{	
			...
			//产品套餐
			$sql = "SELECT * FROM fikcdn_buy WHERE product_id='$buy_id'";
			...
			//域名是否已经存在
			$sql = "SELECT * FROM fikcdn_domain WHERE hostname='$sDomain' ";
			$result = mysql_query($sql,$db_link);
			if($result && mysql_num_rows($result)>0)
			{
	...
	else if($sAction=="modify")
	{
		...
		{	
			...
			/*if($sSrcip!=$domain_upstream || $sUnicomIP!=$unicom_ip || $upstream_add_type!=$upstream_add_all)
			{
				$modify_upstream =1;
			}
			*/
