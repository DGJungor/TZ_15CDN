...
function fiknode_search(){
	...
	window.location.href="user_list3.php?page=1&action=ump"+"&type="+UrlEncode(searchSelect) +"&keyword="+UrlEncode(txtKeyword);
...
<?php	
	...
	if($db_link)
	{
		...
		do
		{
			...
			{
				$sql = "SELECT count(*) FROM fikcdn_client WHERE relation_sale='$relation_sale'"; 
			}
			else
			{
				if($sType=="username")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE username like '%$sKeyword%' AND relation_sale='$relation_sale';" ; 
				}
				else if($sType=="realname")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE realname like '%$sKeyword%' AND relation_sale='$relation_sale';"; 
				}
				else if($sType=="compname")
				{
					$sql = "SELECT count(*) FROM fikcdn_client WHERE company_name like '%$sKeyword%' AND relation_sale='$relation_sale';"; 
				}
			}
			...
			if(strlen($sKeyword)<=0)
			{
				$sql = "SELECT * FROM fikcdn_client WHERE relation_sale='$relation_sale' Limit $offset,$PubDefine_PageShowNum;"; 
			}
			else
			{
				if($sType=="username")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE username like '%$sKeyword%' AND relation_sale='$relation_sale' Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else if($sType=="realname")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE realname like '%$sKeyword%' AND relation_sale='$relation_sale' Limit $offset,$PubDefine_PageShowNum;"; 
				}
				else if($sType=="compname")
				{
					$sql = "SELECT * FROM fikcdn_client WHERE company_name like '%$sKeyword%' AND relation_sale='$relation_sale' Limit $offset,$PubDefine_PageShowNum;"; 
				}
			}
			$result = mysql_query($sql,$db_link);
			if(!$result)
		...
		/*$a=$_SESSION['fikcdn_admin_username'];
		$sql = "SELECT count(*) FROM fikcdn_client WHERE relation_sale='$a'";
			$result = mysql_query($sql,$db_link);
			if($result&&mysql_num_rows($result)>0)
			{
				$total_host  = mysql_result($result,0,"count(*)");	
...
?>	 
		 ...
		 <table width="800" border="0" class="disc">
			...
			<td bgcolor="#FFFFE6"><div class="div_page_bar"> 记录总数：<?php echo $total_host;?>条&nbsp;&nbsp;&nbsp;当前第<?php echo $nPage; ?>页|共<?php echo $total_pages; ?>页&nbsp;&nbsp;&nbsp;跳转
				...
				页&nbsp;&nbsp;&nbsp;&nbsp;<a href="user_list3.php?page=1&action=first<?php echo '&type='.$sType.'&keyword='.$sKeyword; ?>">首页</a>&nbsp;&nbsp;
				<a href="user_list3.php?page=<?php echo $pageup.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pageup">上一页</a>&nbsp;&nbsp;				
				<a href="user_list3.php?page=<?php echo $pagedown.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=pagedown">下一页</a>&nbsp;&nbsp;
				<a href="user_list3.php?page=<?php echo $total_pages.'&type='.$sType.'&keyword='.$sKeyword; ?>&action=tail">尾页 </a></div></td>
			</tr>
		</table>	
	  </td> 
