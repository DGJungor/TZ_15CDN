...
else if($sMod=='order')
{
	if($sAction=="add")
	{
		...
		if($product_id==1&&$month>3){
			$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrParam,'ErrorMsg'=>'本套餐最多可以购买3个月');
			PubFunc_EchoJsonAndExit($aryResult,NULL);
		}
	...
	else if($sAction=="pay")
	{
		...
		if($db_link)
		{
			...
			if($type==$PubDefine_BuyTypeNew)
			{
				...
				$sql = "INSERT INTO fikcdn_buy(relation_sale,sale_group,id,username,product_id,begin_time,end_time,status,auto_renew,price,has_data_flow,domain_num,data_flow,note,dns_cname)
							VALUES('$relation_sale','$sale_group',NULL,'$client_username',$product_id,$timenow,$end_time,1,$auto_renew,$price,$data_flow,$domain_num,$data_flow,'$txtNote','$dns_cname')";
				$result = mysql_query($sql,$db_link);
				if(!$result)
				{
					mysql_query("ROLLBACK",$db_link);
