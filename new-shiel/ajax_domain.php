...
if($sMod=='domain')
{
	if($sAction=="add")
	{
		...
		{	
			...
			$sale_group = mysql_result($result,0,"sale_group");
			//产品套餐
			...
			$timenow = time();
			$sql="INSERT INTO fikcdn_domain(relation_sale,sale_group,id,hostname,username,add_time,buy_id,group_id,upstream,unicom_ip,icp,DNSName,status,begin_time,end_time,note,upstream_add_all,SSLOpt,SSLCrtContent,SSLKeyContent,SSLExtraParams,UpsSSLOpt) 
					VALUES('$relation_sale','$sale_group',NULL,'$sDomain','$client_username',$timenow,$buy_id,$group_id,'$sSrcip','$sUnicomIP','$sIcp','$dns_name','$domain_status',0,0,'$sBackup',$upstream_add_type,'$SSLOpt','$SSLCrtContent','$SSLKeyContent','$SSLExtraParams','$UpsSSLOpt')";
			$result = mysql_query($sql,$db_link);
			if(!$result)
			{
				echo $sql;
	...
	else if($sAction=="modify")
	{
		...
		{	
			...
			//如果源站IP无修改或域名还在审核中则不需要修改15CDN服务器
