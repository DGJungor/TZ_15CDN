...
else if($sMod=='order')
{
	...
	else if($sAction=="pay")
	{
		...
		if($db_link)
		{
			...
			$new_cname = mysql_result($result,0,"dns_cname");
			
			//bendy add 获取独有的cname
			require_once '../vendor/autoload.php';
			$dns_api = new \CloudXNS\Api();
			$dns_api->setApiKey($xns['api']);
			$dns_api->setSecretKey($xns['key']);
			//$dns_api->setProtocol(true);
			$dns_api_message=json_decode($dns_api->record->recordList($xns['domainid'], 0, 0, 300),true);
			if ($dns_api_message['code'] <> 1){
				//error
				$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrNoUser,'ErrorMsg'=>'无法连接DNS服务商,请稍候重试,或者联系管理员。'.$dns_api_message['message']);
				PubFunc_EchoJsonAndExit($aryResult,$db_link);
			}
			$records=$dns_api_message['data'];
			...
			foreach ($records as $r){
				if ($code == $r['host']) {
					//fined! changeit!
					$find_host=True;
					$dns_api_message=json_decode($dns_api->record->recordUpdate($xns['domainid'], $code, $new_cname, 'CNAME', 10, 600, 1, '', $r['record_id']),true);
					if ($dns_api_message['code'] <> 1){
						$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrNoUser,'ErrorMsg'=>'无法更新DNS记录。'.$dns_api_message['message']);
						PubFunc_EchoJsonAndExit($aryResult,$db_link);
					}
					break;
				}
			...
			if ($find_host == False){
				$dns_api_message=json_decode($dns_api->record->recordAdd($xns['domainid'], $code, $new_cname, 'CNAME', 10, 600, 1),true);
				if ($dns_api_message['code'] <> 1){
					$aryResult = array('Return'=>'False','ErrorNo'=>$PubDefine_ErrNoUser,'ErrorMsg'=>'无法插入DNS记录。'.$dns_api_message['message']);
					PubFunc_EchoJsonAndExit($aryResult,$db_link);
				}
			}
			$dns_cname=$code.'.'.$xns['domainname'];
			//end bendy
