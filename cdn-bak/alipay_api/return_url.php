...
include_once('../../function/pub_func.php');
include_once("../../function_agent.php");
...
if($verify_result) {//验证成功
		...
		if(!$db_link)
		{
			...
			if($status==1)
			{
				...
				if($result2 && mysql_num_rows($result2)>0)
				{
					...
					$note			=mysql_result($result,0,"note");
					$curmoney		=mysql_result($result,0,"money");
				...
				//充值
				$sql="UPDATE fikcdn_client SET money=money+$money WHERE username='$username';";			
				$result = mysql_query($sql,$db_link);
				if(!$result)
				{   
				...
				if($result2 && mysql_num_rows($result2)>0)
				{
					...
					$sql="UPDATE fikcdn_recharge SET status='0',time='$pay_time' WHERE order_id='$out_trade_no';";			
					$result = mysql_query($sql,$db_link);
					if(!$result)
					{   
						mysql_query("ROLLBACK",$db_link);   
	...
	echo '在线支付成功，<a href="../main.php">返回平台</a>&nbsp;查看是否已经在线充值成功，如果有任何问题，请和工作人员联系。<br />';
